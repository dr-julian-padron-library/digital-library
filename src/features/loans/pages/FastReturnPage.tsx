...
import { useFastReturnMutation } from "../api/readingSessionsApiSlice";
...
import { toast } from "sonner"; // Using 'sonner' as per FastCheckoutPage

export default function FastReturnPage() {
    const [userId, setUserId] = useState("");
    const [cota, setCota] = useState("");
    const [fastReturn, { isLoading }] = useFastReturnMutation();
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            await fastReturn({ user_id: userId, cota }).unwrap();
            ...
            // Reset
            setCota("");
            // We keep the userId mostly cleared or maybe we want to return multiple books for same user?
            // "Fast Return" usually matches "Fast Checkout" logic. 
            // In FastCheckoutPage I noted "Avant-Garde decision: Minimalism. Clear everything."
            // I'll stick to that consistency unless requested.
            setUserId("");
            ...
            console.error(err);
            const errorMessage = err.data?.detail || "Error al registrar la devolución.";
            toast.error("Error en la devolución", {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 fade-in animate-in zoom-in-95 duration-500">
            <div className="relative w-full max-w-md">
                {/* Decorative elements for Avant-Garde feel - slightly different color for Return? Maybe Green/Teal hint */}
                <div className="absolute -top-10 -left-10 w-24 h-24 bg-green-500/10 rounded-full blur-3xl" />
                ...
                <Card className="border-border/50 bg-background/50 backdrop-blur-xl shadow-2xl relative overflow-hidden">
                    ...
                    <CardHeader className="text-center pb-2">
                        ...
                        <CardTitle className="text-2xl font-bold tracking-tight">Devolución Rápida</CardTitle>
                    ...
                    <CardContent className="space-y-6 pt-6">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="12345"
                                className="font-mono text-lg tracking-widest bg-background/50 border-green-500/20 focus:border-green-500 transition-all duration-300"
                                autoComplete="off"
                            />
                        </div>
                        ...
                        <Button
                            ...
                            disabled={isLoading}
                            className="w-full h-12 text-md font-semibold mt-4 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] bg-green-600 hover:bg-green-700 text-white"
                        >
                            {isLoading ? (
                                <>
